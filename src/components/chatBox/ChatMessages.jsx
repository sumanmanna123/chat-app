    ...
    <div className="flex flex-col space-y-4">
      {messages.map((msg) => {
        ...
        return (
          ...
          >
            <div
              className={`max-w-xs p-3 rounded-lg ${isMine ? 'bg-blue-100' : 'bg-gray-100'}`}
            >
              {!isMine && <p className="text-sm font-semibold mb-1">{getUsername(msg.sender)}</p>}
              ...
              {isMine && <p className="text-sm font-semibold mb-1 text-right">You</p>}
            </div>
          </div>
        );
      })}
